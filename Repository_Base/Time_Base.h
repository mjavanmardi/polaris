...
namespace Time_Components
{
	namespace Bases
	{
		...
		struct Time_Base
		{
			...
			feature_implementation void Initialize(TargetType t, call_requirements(requires(ComponentType, Is_Dispatched) && requires(TargetType,Concepts::Time_In_DRSeconds)))
			{
				this->_local_time=((float)t/ (float)10.0);
			}
			/// Initialize Time using a Seconds struct
			feature_implementation void Initialize(TargetType t, call_requirements(requires(ComponentType, Is_Dispatched) && requires(TargetType,Concepts::Time_In_Seconds)))
			{
				this->_local_time=((float)t);
			}
			/// Initialize Time using a Minutes struct
			feature_implementation void Initialize(TargetType t, call_requirements(requires(ComponentType, Is_Dispatched) && requires(TargetType,Concepts::Time_In_Minutes)))
			{
				this->_local_time=((float)t * (float)60.0);
			}
			/// Initialize Time using an Hours struct
			feature_implementation void Initialize(TargetType t, call_requirements(requires(ComponentType, Is_Dispatched) && requires(TargetType,Concepts::Time_In_Hours)))
			{
				this->_local_time=((float)t * (float)3600.0);
			}
			/// Initialize Time using a Days struct
			feature_implementation void Initialize(TargetType t, call_requirements(requires(ComponentType, Is_Dispatched) && requires(TargetType,Concepts::Time_In_Days)))
			{
			...
			feature_implementation void Initialize(TargetType t, call_requirements(
			...
			feature_implementation TargetType Time_Component(call_requirements(requires(ComponentType, Is_Dispatched) && requires(TargetType,Concepts::Time_In_Days)))
			{
				return (TargetType)((int)(_local_time/86400.0));
			}
			feature_implementation TargetType Time_Component(call_requirements(requires(ComponentType, Is_Dispatched) && requires(TargetType,Concepts::Time_In_DRSeconds)))
			{
				return (TargetType)((int)(_local_time*10)%10);
			}
			feature_implementation TargetType Time_Component(call_requirements(requires(ComponentType, Is_Dispatched) && requires(TargetType,Concepts::Time_In_Hours)))
			{
				return (TargetType)((int)(_local_time/3600.0) % 24);
			}
			feature_implementation TargetType Time_Component(call_requirements(requires(ComponentType, Is_Dispatched) && requires(TargetType,Concepts::Time_In_Minutes)))
			{
				return (TargetType)((int)(_local_time/60.0)%60);
			}
			feature_implementation TargetType Time_Component(call_requirements(requires(ComponentType, Is_Dispatched) && requires(TargetType,Concepts::Time_In_Seconds)))
			{
				return (TargetType) ((int)(_local_time)%60);
			}
			feature_implementation void Time_Component(call_requirements(
				requires(ComponentType, !Is_Dispatched) || (requires(TargetType,!Concepts::Time_In_DRSeconds) &&	requires(TargetType,!Concepts::Time_In_Seconds) && 
				requires(TargetType,!Concepts::Time_In_Minutes) && requires(TargetType,!Concepts::Time_In_Hours) && requires(TargetType,!Concepts::Time_In_Days))))
			{
				assert_requirements(ComponentType, Is_Dispatched, "ComponentType is not dispatched.");
			...
			feature_implementation void Total_Time(call_requirements(
				requires(ComponentType, !Is_Dispatched) || (requires(TargetType,!Concepts::Time_In_DRSeconds) &&	requires(TargetType,!Concepts::Time_In_Seconds) && 
				requires(TargetType,!Concepts::Time_In_Minutes) && requires(TargetType,!Concepts::Time_In_Hours) && requires(TargetType,!Concepts::Time_In_Days))))
			{
				assert_requirements(ComponentType, Is_Dispatched, "ComponentType is not dispatched.");
			...
			//------------------------------------------------------------------------------------------------
			member_data(float, local_time, requires(TargetType,is_arithmetic),requires_2(ComponentType,CallerType,Is_Same_Component) && requires(TargetType, is_arithmetic));		
		};
	}
