...
namespace Demand_Components
{
	...
	namespace Prototypes
	{
		...
		struct Demand_Prototype
		{
			feature_accessor(scenario_reference);
			feature_accessor(network_reference);

			feature void read_demand_data()
			{
				...
				define_component_interface_local(_Network_Interface, Network_Prototype, network_reference, ComponentType);
				
				define_container_and_value_interface(Polaris_Random_Access_Sequence_Prototype, _Network_Interface_type::activity_locations_container, _Activity_Locations_Container_Interface, Activity_Location_Prototype, _Activity_Location_Interface, ComponentType);
				define_container_and_value_interface(Polaris_Random_Access_Sequence_Prototype, _Activity_Location_Interface_type::origin_links, _Links_Container_Interface, Link_Prototype, _Link_Interface, ComponentType);

				define_component_interface(_Vehicle_Interface, Vehicle_Prototype, ComponentType::traveler_type::vehicle, ComponentType);
				define_component_interface(_Routing_Interface, Routing_Prototype, ComponentType::traveler_type::router, ComponentType);
					...
					for (int i=0;i<scenario_reference<_Scenario_Interface*>()->simulation_interval_length<int>();i++)
					{
						...
						while(cur_vehicle_rate_per_simulation_interval>0)
						{
							if (cur_vehicle_rate_per_simulation_interval>=1.0)
							{
								...
								vehicle=(_Vehicle_Interface*)Allocate<_Vehicle_Interface_type>();
								router=(_Routing_Interface*)Allocate<_Routing_Interface_type>();
					...
					for (int i=0;i<scenario_reference<_Scenario_Interface*>()->simulation_interval_length<int>();i++)
					{
						...
						while(cur_vehicle_rate_per_simulation_interval>0)
						{
							if (cur_vehicle_rate_per_simulation_interval>=1.0)
							{
								traveler=(_Traveler_Interface*)Allocate<ComponentType::traveler_type>();
								vehicle=(_Vehicle_Interface*)Allocate<_Vehicle_Interface_type>();
								router=(_Routing_Interface*)Allocate<_Vehicle_Interface_type>();
