	...
	namespace Bases
	{
		...
		protected:
			...
			void Initialize(InputFileType& input_file_struct, TimeStructType& iteration_step_struct, call_requirements(
				...
				requires(TargetType::Param2Type TripType,Trip_Components::Concepts::Is_Trip) &&
			...
			void Initialize(InputFileType& input_file_struct, TimeStructType& iteration_step_struct, call_requirements(
				requires(ThisType,Is_Dispatched) &&
				requires(TripType,Trip_Components::Concepts::Is_Trip) &&
				...
				requires(InputFileType,!Concepts::Is_Transims_Trip_File))))
			{
				...
				assert_requirements(TripType, Trip_Components::Concepts::Is_Trip, "Your TripType type does not satisfy the requirements of a Trip - ");
			...
			facet void push_trip(Trip_Info* trip, call_requirements(requires(ThisType, Is_Dispatched) && requires(TargetType, Trip_Components::Concepts::Is_Trip) /*&& requires(ThisType,Is_Same_Component)*/))
			{
				Trip_Components::Interfaces::Trip_Interface* new_trip = Allocate<Trip_Components::Components::Trip>();

				Time_Components::Data_Structures::Time_Seconds t;
				t.Time = (Time_Components::Data_Structures::Time_Seconds::ValueType)trip->Departure_Time.Total_Seconds();

				Location_Components::Data_Structures::Location_Struct o, d;
				o.location = trip->Start_Location;
				d.location = trip->End_Location;

				new_trip->Initialize<Trip_Components::Components::Trip,ThisType,int,Time_Components::Data_Structures::Time_Seconds,Location_Components::Data_Structures::Location_Struct>(trip->hh_id * 100 + trip->vehicle_id,t,t,o,d);
				_current_trips.push_back(new_trip);

			}
			/// Error handler for push_trip functionality
			facet void push_trip(Trip_Info* trip, call_requirements(requires(ThisType, !Is_Dispatched) || requires(TargetType, !Trip_Components::Concepts::Is_Trip) ))
			{
			...
			facet void Print_Trips(call_requirements(requires(ThisType, Is_Dispatched) && requires(TargetType, Trip_Components::Concepts::Is_Trip)))
			{
				for (trip_itr = _current_trips.begin(); trip_itr != _current_trips.end(); trip_itr++)
				{
			...
			Time_Components::Interfaces::Time_Interface* _timestep_start;
			tag_getter_setter(timestep_start);
			facet void timestep_start(typename TargetType::Interface_Type* set_value, call_requirements(requires(ThisType, Is_Dispatched) && requires(ThisType,Is_Same_Component) && requires(TargetType, Time_Components::Concepts::Is_Time)))
			{
				_timestep_start = (Time_Components::Interfaces::Time_Interface*)set_value;
			}
			facet void timestep_start(typename TargetType::Interface_Type*  set_value, call_requirements(requires(ThisType, !Is_Dispatched) || requires(ThisType,!Is_Same_Component) || requires(TargetType, !Time_Components::Concepts::Is_Time)))
			{ 
				assert_requirements(ThisType, Is_Dispatched,"ThisType is not dispatched");
				assert_requirements(ThisType, Is_Same_Component, "ThisType does not have access, because it is not the same component.");
				assert_requirements(TargetType, Time_Components::Concepts::Is_Time,"TargetType is not a valid TimeType");
			}
			facet typename TargetType::Interface_Type* timestep_start(call_requirements(requires(ThisType, Is_Dispatched)&& requires(TargetType, Time_Components::Concepts::Is_Time)))
			{ 
			...
			Time_Components::Interfaces::Time_Interface* _timestep_end;
			tag_getter_setter(timestep_end);
			facet void timestep_end(typename TargetType::Interface_Type* set_value, call_requirements(requires(ThisType, Is_Dispatched) && requires(ThisType,Is_Same_Component) && requires(TargetType, Time_Components::Concepts::Is_Time)))
			{
				_timestep_end = (Time_Components::Interfaces::Time_Interface*)set_value;
			}
			facet void timestep_end(typename TargetType::Interface_Type*  set_value, call_requirements(requires(ThisType, !Is_Dispatched) || requires(ThisType,!Is_Same_Component) || requires(TargetType, !Time_Components::Concepts::Is_Time)))
			{ 
				assert_requirements(ThisType, Is_Dispatched,"ThisType is not dispatched");
			...
			facet typename TargetType::Interface_Type* timestep_end(call_requirements(requires(ThisType, Is_Dispatched)&& requires(TargetType, Time_Components::Concepts::Is_Time)))
			{ 
			...
			/// File format
			string _file_format;
			vector<Trip_Components::Interfaces::Trip_Interface*> _current_trips;
			vector<Trip_Components::Interfaces::Trip_Interface*>::iterator trip_itr;
			//------------------------------------------------------------------------------------------------------------------
