...
void PopulateEvent(std::string db_path)
{
	...
	try
	{
		...
		instance.reset(new Event_Instance(0, links, (float)point.x, (float)point.y, "Database", "5",-1, 2*3600, false, 9*3600, 11*3600, "Snow in the lake county"));  evnt->setInstanc(instance); instance->setEvent(evnt); db->persist(instance);
		...
		instance.reset(new Event_Instance(0, links, (float)point.x, (float)point.y, "Database", "5",-1, -1, false, 10*3600, 11*3600, "Accident on I-55"));  evnt->setInstanc(instance); instance->setEvent(evnt); db->persist(instance);
		...
		instance.reset(new Event_Instance(0, links, (float)point.x, (float)point.y, "Database", "5",-1, -1, false, 10.5*3600, 11.5*3600, "Conjestion caused by an accident on I-55"));  evnt->setInstanc(instance); instance->setEvent(evnt); db->persist(instance);
		...
		instance.reset(new Event_Instance(0, links, (float)point.x, (float)point.y, "Database", "5",-1, -1, false, 0*3600, 14*3600, "Lame closure due to construction on I-55"));  evnt->setInstanc(instance); instance->setEvent(evnt); db->persist(instance);
