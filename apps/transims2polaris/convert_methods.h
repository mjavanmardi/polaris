...
int AddGeometryTables(sqlite3* db_handle, int srid)
{
	...
	ret = polaris::io::AddGeometryCoumn(db_handle, srid, "LINK", "LINESTRING", "XY");
	if (ret != SQLITE_OK) goto stop;
	ret = polaris::io::AddGeometryCoumn(db_handle, srid, "Zone", "POINT", "XY");
	if (ret != SQLITE_OK) goto stop;
	ret = polaris::io::AddGeometryCoumn(db_handle, srid, "Node", "POINT", "XY");
	if (ret != SQLITE_OK) goto stop;
stop:
	return ret;
}
void AddSpatialiteGeometry(TransimsNetwork *net)
{
	...
	if (shapes.size() != 0)
	{
		...
		strcpy (sql, buff);
		ret = sqlite3_prepare_v2 (db_handle, sql, (int)strlen (sql), &stmt, NULL);
		if (ret != SQLITE_OK)
		{
		...
		{	
			...
			sqlite3_bind_text (stmt, 1, geometry.c_str(), (int)strlen (geometry.c_str()), SQLITE_STATIC);
