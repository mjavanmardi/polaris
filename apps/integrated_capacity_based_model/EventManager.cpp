...
void EventManager::setupTimeStep()
{
	...
	for(interIt it = network->begin(); it != network->end();it++)
	{
		...
		vector<CMEvent> eventsToAdd = (*it)->getPossibleEvents();
...
void EventManager::runTimeStep()
{
	while(!eventStack.empty())
	{
		...
		CMEvent currentEvent = eventStack.top();
		eventStack.pop();
		...
		if(currentIntersection != NULL)
		{
			...
			vector<CMEvent> toReschedule = currentIntersection->runEvent(currentEvent,time,dt);
			//Rescheduling the events that needs to be
			for(vector<CMEvent>::iterator it = toReschedule.begin();it != toReschedule.end();it++)
