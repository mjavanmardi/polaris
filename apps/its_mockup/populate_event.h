...
void PopulateEvent(std::string db_path)
{
	...
	try
	{
		...
		instance.reset(new Event_Instance(0, links, (float)point.x, (float)point.y, "Database", "5",-1, -1, false, (int)8*3600, (int)8.5*3600, "Accident on LSD"));  evnt->setInstanc(instance); instance->setEvent(evnt);

		point_map = GetLinkPoint(db_path, 16899);
		point = point_map.begin()->second;
		links.clear();
		links.push_back(16899);
		instance1.reset(new Event_Instance(0, links, (float)point.x, (float)point.y, "Database", "5",-1, -1, false, (int)6*3600, (int)6.5*3600, "Accident on I-290"));  evnt->setInstanc(instance1); instance1->setEvent(evnt);
		...
		instance.reset(new Event_Instance(0, links, (float)point.x, (float)point.y, "Database", "5",-1, -1, false, (int)10.5*3600, (int)11.5*3600, "Congestion caused by an accident on I-55"));  evnt->setInstanc(instance); instance->setEvent(evnt); 
		...
		instance.reset(new Event_Instance(0, links, (float)point.x, (float)point.y, "Database", "5",-1, -1, false, (int)0*3600, (int)14*3600, "Lane closure due to construction on I-55"));  evnt->setInstanc(instance); instance->setEvent(evnt); 
