...
namespace polaris
{
	...
	struct Base_Edge_A_Star : public set_base_hook< optimize_size<false> >
	{
		...
		Base_Edge_A_Star():_x(0.0f),_y(0.0f),_source_link(nullptr),_cost_from_origin(FLT_MAX / 2.0f),_time_label(0.0f),
					...
					_cost(0.0f),_time_cost(0.0f),_walk_distance_to_transit(FLT_MAX / 2.0f),
					_drive_fft_to_transit(FLT_MAX / 2.0f),_touch_transit(false) {}
		...
		t_data(float, walk_distance_to_transit);
