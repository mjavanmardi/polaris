...
namespace PopSyn
{
	...
	{
		prototype struct Population_Unit : public ComponentType
		{
			tag_as_prototype;
			typedef Population_Unit<ComponentType> This_Type;
			
			accessor(ID,check(strip_modifiers(TargetType),is_arithmetic),check(strip_modifiers(TargetType),is_arithmetic));
			accessor(Weight,check(strip_modifiers(TargetType),is_arithmetic),check(strip_modifiers(TargetType),is_arithmetic));
			accessor(Index,check(strip_modifiers(TargetType),is_arithmetic),check(strip_modifiers(TargetType),is_arithmetic));
			...
			template<typename TargetType> void Initialize(TargetType object_to_copy/*, requires(TargetType,check_2(TargetType,This_Type,is_same))*/)
			{
				...
				typedef typename get_type_of(Characteristics)::value_type value_type;
				typedef Random_Access_Sequence<typename get_type_of(Characteristics),value_type> characteristics_itf;
