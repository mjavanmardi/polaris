...
namespace RNG_Components
{
	...
	namespace Concepts
	{
		...
		{
			check_template_method_name(Has_Rand_Defined,Next_Rand);
			check_template_method_name(Has_Rand_Defined_Prototype, Component_Type::Next_Rand);
	...
	namespace Prototypes
	{
		prototype struct RNG ADD_DEBUG_INFO
		{
			...
			template<typename TargetType> void Initialize(	TargetType seed_value,
												...
												requires(TargetType,check(typename remove_pointer<TargetType>::type,!is_arithmetic)))
			{
				assert_check(typename remove_pointer<TargetType>::type,is_arithmetic,"Error, the specified TargetType does not satisfy the 'is_arithmetic' concept");
			}
