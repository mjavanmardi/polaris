...
namespace Person_Components
{
	namespace Implementations
	{
		...
		implementation struct ADAPTS_Person_Properties_Implementation : public Polaris_Component<MasterType,INHERIT(ADAPTS_Person_Properties_Implementation),Data_Object>
		{
			...
			// static counters
			static int Count_Array[num_sim_threads()];
			static int Count;
			...
			template<typename TargetType> void Initialize(typename TargetType::ParamType home_synthesis_zone/*, requires(TargetType,check(typename TargetType::ParamType, PopSyn::Concepts::Is_Synthesis_Zone) && check(typename TargetType::ParamType, is_pointer))*/)
			{	
			...
			/*template<typename TargetType> void Initialize(typename TargetType::ParamType home_synthesis_zone, requires(TargetType,!check(typename TargetType::ParamType, Zone_Components::Concepts::Is_Zone_Prototype) || !check(typename TargetType::ParamType, is_pointer)))
			{	
			...
			template<typename TargetType> typename TargetType::ReturnType Average_Activity_Duration(ACTIVITY_TYPES act_type)
			{
				Time_Minutes value;
...
//TODO
//				duration = Time<Basic_Units::Implementations::Time_Implementation<NT>>::Convert_Value<Target_Type<NT,typename TargetType::ReturnType,Time_Minutes>>(value);
				return duration;
			}
			tag_feature_signature_as_available(Average_Activity_Duration,1);
			template<typename TargetType> void Average_Activity_Duration(typename TargetType::ParamType act_type, typename TargetType::Param2Type value)
			{
				// duration stored in minutes
//TODO
//				Time_Minutes duration = Time<Basic_Units::Implementations::Time_Implementation<NT>>::Convert_Value<Target_Type<NT,Time_Minutes,typename TargetType::Param2Type>>(value);
			...
			template<typename TargetType> typename TargetType::ReturnType Average_Activity_Frequency(ACTIVITY_TYPES act_type)
			{
				typename average_activity_frequency_and_duration_container_type::iterator itr;
				itr = this->_average_activity_frequency_and_duration_container.find(act_type);
				if (itr != this->_average_activity_frequency_and_duration_container.end())
			...
			template<typename TargetType> void Average_Activity_Frequency(typename TargetType::ParamType act_type, typename TargetType::Param2Type value)
			{
				typename average_activity_frequency_and_duration_container_type::iterator itr;
				itr = this->_average_activity_frequency_and_duration_container.find(act_type);
				if (itr != this->_average_activity_frequency_and_duration_container.end())
		...
		template<typename MasterType, typename InheritanceList> Pair_Associative_Container<boost::unordered::unordered_map<int, pair<float,float>>> ADAPTS_Person_Properties_Implementation<MasterType,  InheritanceList>::_average_activity_frequency_and_duration_container;

		implementation struct ACS_Person_Static_Properties_Implementation : public Polaris_Component< MasterType,INHERIT(ACS_Person_Static_Properties_Implementation), Data_Object, ParentType>
		{
			// Tag as Implementation
