...
namespace Prototypes
{
	...
	{
		tag_as_prototype;
		template<typename TargetType> void Add_Record(TargetType act_record, bool is_executed, requires(TargetType,check(strip_modifiers(TargetType),is_pointer) /*&& check(strip_modifiers(TargetType),Activity_Components::Concepts::Is_Activity_Plan_Prototype)*/))
		{
			this_component()->Add_Record<TargetType>(act_record,is_executed);
		}
		template<typename TargetType> void Add_Record(TargetType act_record, bool is_executed, requires(TargetType,!check(strip_modifiers(TargetType),is_pointer)/* || !check(strip_modifiers(TargetType),Activity_Components::Concepts::Is_Activity_Plan_Prototype)*/))
		{
			assert_check(strip_modifiers(TargetType),is_pointer,"Target type is not a pointer");
