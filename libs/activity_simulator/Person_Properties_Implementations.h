	...
	namespace Implementations
	{
		...
		implementation struct ADAPTS_Person_Properties_Implementation : public Polaris_Component<MasterType,INHERIT(ADAPTS_Person_Properties_Implementation),Data_Object>
		{
			...
			{	
				...
				typedef Prototypes::Person_Properties<typename Parent_Person_type::get_type_of(Static_Properties)> pop_unit_itf;
				
				// useful interfaces
				typedef Random_Access_Sequence<typename zone_itf::get_type_of(Activity_Locations_Container),int> activity_location_ids_itf;
				typedef Network_Components::Prototypes::Network<typename Parent_Person_type::get_type_of(network_reference)> network_itf;
				typedef Person_Planner<typename Parent_Person_type::get_type_of(Planning_Faculty)> planner_itf;
