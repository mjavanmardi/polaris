...
struct Intrusive_Binary_Heap
{
	...
	{		
		...
		//if(a->parent == b) swap_type = true;

		const bool parent_swap = (a->parent == b);

		//swap_type = false;
		//if(a->row_left == b) swap_type = true;
	...
	void heapify_down(ibh_node* val)
	{
		while(true)
		{
			//int val_swap = val->value;
			//int val_left = INT_MAX;
			//int val_right = INT_MAX;
			...
			if(swap_right && swap_left)
			{
				...
				//swap_left = (*((T*)val->right_child) < *((T*)val->left_child));
			}
