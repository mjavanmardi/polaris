...
//				_link_queue_length_layer=Allocate_New_Layer< typename MasterType::type_of(canvas),NT,Target_Type< NULLTYPE,Antares_Layer<type_of(link_queue_length_layer)>*, string& > >(string("Link Queue Length"));
			...
			static bool submit_attributes(Antares_Link_Implementation* _this,boost::container::vector<string>& bucket)
			{
				// not supported yet
				return true;
			}
...
#pragma pack(pop)
			...
			static bool fetch_attributes(Antares_Link_Implementation* _this,boost::container::vector<string>& bucket)
			{
				_this->plot_link_moe();
