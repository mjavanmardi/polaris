...
namespace PopSyn
{
	...
	namespace Implementations
	{
		...
		implementation struct _Popgen_Synthesis_Zone_Implementation : public _Synthesis_Zone_Base_Implementation<MasterType>
		{
			...
			//?????????????????????????????????????????????????????????????????????????????????????????????????????
			// UPDATE: Containers for holding the joint and marginal distributions - these should be specific to popgen
			//-----------------------------------------------------------------------------------------------------
			/*m_container(m_array<double>,Target_Joint_Distribution, NONE,NONE);
			m_container(s_array<double>,Target_Marginal_Distribution, NONE, NONE);
			m_container(m_array<double>,Target_Person_Joint_Distribution, NONE,NONE);
			...
			//------------------------------------------------------------------------------------------
			// Initializers which creates and intializes the data containers based on the stated dimensions of the househodl and person control variables
			...
			// This is the primary PopGen fitting function - implement routine to do the CO here
			template<typename AnalysisUnitType> void Fit_Joint_Distribution_To_Marginal_Data()
			{
				typedef Scenario_Components::Prototypes::Scenario<typename MasterType::scenario_type> _Scenario_Interface;
				_Scenario_Interface* scenario = this->scenario_reference<_Scenario_Interface*>();
			...
			// member functions specific to the CO routine - add as needed
		};
