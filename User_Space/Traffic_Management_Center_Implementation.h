	...
	namespace Implementations
	{
		implementation struct Simple_TMC:public Polaris_Component<APPEND_CHILD(Simple_TMC),MasterType,Execution_Object>
		{
			...
			feature_implementation void Initialize()
			{
				...
				execute_static_typelist_loop(Initialize_List,MasterType::its_component_types,nullptr);
			}
			...
			feature_implementation void Read_Database()
			{
				...
				for(result<Component>::iterator db_itr = component_result.begin (); db_itr != component_result.end (); ++db_itr)
				{
					...
					{
						Variable_Speed_Sign_Interface::Setup_Type<const vector<shared_ptr<Component_Key>>&>(db_itr->getKeys());
					}
					else if(name == "Variable Message Sign")
					{
						Variable_Word_Sign_Interface::Setup_Type<const vector<shared_ptr<Component_Key>>&>(db_itr->getKeys());
					}
					else if(name == "Tow Truck Depot")
					{
						Depot_Interface::Setup_Type<const vector<shared_ptr<Component_Key>>&>(db_itr->getKeys());
					}
					else if(name == "HAR")
					{
						Advisory_Radio_Interface::Setup_Type<const vector<shared_ptr<Component_Key>>&>(db_itr->getKeys());
					}
					else if(name == "Open Shoulder")
					{
						Link_Control_Interface::Setup_Type<const vector<shared_ptr<Component_Key>>&>(db_itr->getKeys());
					}
					...
					for(vector<weak_ptr<Instance>>::const_iterator vitr=instances.begin();vitr!=instances.end();vitr++)
					{
						...
						if(name == "Variable Speed Sign")
						{
							...
							its_component->Setup< weak_ptr<Instance>& >(_ptr);
