...
namespace Person_Components
{
	...
	namespace Prototypes
	{
		prototype struct Person_Properties ADD_DEBUG_INFO
		{
			...
			define_feature_exists_check(Normalize_Weight, Has_Normalize);
			feature_prototype void Normalize_Weight(TargetType normalization_factor, requires(check(TargetType,is_arithmetic) && check(ComponentType,Has_Normalize)))
			{
				this->Weight<TargetType>(this->Weight<TargetType>()/normalization_factor);
			}
			feature_prototype void Normalize_Weight(TargetType normalization_factor, requires(!check(TargetType,is_arithmetic) || !check(ComponentType,Has_Normalize)))
			{
				assert_check(ComponentType,Has_Initialize,"This ComponentType is not a valid Static_Property, does not have a Normalizer.   Did you forget to use tag_feature_as_available macro?");
			}
