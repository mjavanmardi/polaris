		...
		implementation struct Polaris_Network_Implementation:public Polaris_Component_Class<Polaris_Network_Implementation,MasterType,Execution_Object,ParentType>
		{
			...
			feature_implementation void initialize_links()
			{
				...
				for (links_itr = _links_container.begin(); links_itr != _links_container.end(); links_itr++)
				{
					...
					link->template link_fftt_cached_simulation_interval_size<int>(int(ceil(float(fftt/(float(((_Scenario_Interface*)_global_scenario)->template simulation_interval_length<float>()))))));
					link->template link_bwtt_cached_simulation_interval_size<int>(int(ceil(float(bwtt/(float(((_Scenario_Interface*)_global_scenario)->template simulation_interval_length<float>()))))));
			...
			// these are called by printResults; move to implementation
			...
			feature_implementation void read_network_data(typename TargetType::ParamType net_io_maps,
			...
			feature_implementation void read_network_data(typename TargetType::ParamType network_data, requires(check_2(TargetType::NetIOType,Types::File_Network,is_same)))
			{
				define_container_and_value_interface(_Intersections_Container_Interface, _Intersection_Interface, type_of(intersections_container), Random_Access_Sequence_Prototype, Intersection_Components::Prototypes::Intersection_Prototype, ComponentType);
				define_container_and_value_interface(_Links_Container_Interface, _Link_Interface, type_of(links_container), Random_Access_Sequence_Prototype, Link_Components::Prototypes::Link_Prototype, ComponentType);
				define_container_and_value_interface(_Turn_Movements_Container_Interface, _Turn_Movement_Interface, type_of(turn_movements_container), Random_Access_Sequence_Prototype, Intersection_Components::Prototypes::Movement_Prototype, ComponentType);
