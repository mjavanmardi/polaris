	...
	namespace Prototypes
	{
		prototype struct Movement_Prototype
		{
			...
			feature_prototype TargetType offer_vehicle()
			{
				...
				define_container_and_value_interface(_Vehicles_Container_Interface, _Vehicle_Interface, get_type_of(vehicles_container), Back_Insertion_Sequence_Prototype, Vehicle_Prototype, ComponentType);
		...
		prototype struct Intersection_Prototype
		{
			...
			feature_prototype void accept_vehicle(void* vehicle)
			{
				...
				define_container_and_value_interface(_Outbound_Inbound_Movements_Container_Interface, _Outbound_Inbound_Movements_Interface, get_type_of(outbound_inbound_movements), Random_Access_Sequence_Prototype, Outbound_Inbound_Movements_Prototype, ComponentType);
				define_component_interface(_Link_Interface, _Outbound_Inbound_Movements_Interface::get_type_of(outbound_link_reference), Link_Prototype, ComponentType);
				define_container_and_value_interface(_Movements_Container_Interface, _Movement_Interface, _Outbound_Inbound_Movements_Interface::get_type_of(inbound_movements), Random_Access_Sequence_Prototype, Movement_Prototype, ComponentType);
				define_container_and_value_interface(_Vehicles_Container_Interface, _Vehicle_Interface, _Movement_Interface::get_type_of(vehicles_container), Back_Insertion_Sequence_Prototype, Vehicle_Prototype, ComponentType);
				define_component_interface(_Intersection_Interface, _Link_Interface::get_type_of(upstream_intersection), Intersection_Prototype,  ComponentType);
			...
			declare_feature_conditional(Newells_Conditional)
			{
				...
				if(intersection_current_revision._iteration!=_iteration)
				{
				...
				if(_this_ptr->intersection_simulation_status<Types::Intersection_Simulation_Status>()==Types::Intersection_Simulation_Status::NONE_COMPLETE)
				{
					...
					if(links_current_revision._iteration==_iteration)
					{
				...
				else if(_this_ptr->intersection_simulation_status<Types::Intersection_Simulation_Status>()==Types::Intersection_Simulation_Status::COMPUTE_STEP_FLOW_COMPLETE)
				{
					...
					if(link_next_revision._iteration>_iteration)
					{
						...
						response.result=true;
						response.next=_iteration + _this_ptr->scenario_reference<_Scenario_Interface*>()->simulation_interval_length<int>();
					}
					else
					{
