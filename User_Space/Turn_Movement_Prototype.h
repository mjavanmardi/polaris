...
namespace Turn_Movement_Components
{
	...
	namespace Prototypes
	{
		prototype struct Movement_Prototype
		{
			...
			{
				this_component()->update_capacity<ComponentType,CallerType,TargetType>(param);
			}

			feature_prototype void update_flow()
			{
				this_component()->update_flow<ComponentType,CallerType,TargetType>();
			}

			feature_prototype void transfer_vehicles(TargetType param)
			{
				this_component()->transfer_vehicles<ComponentType,CallerType,TargetType>(param);
			}

			feature_prototype void update_state(TargetType param)
			{
				this_component()->update_state<ComponentType,CallerType,TargetType>(param);
			}
